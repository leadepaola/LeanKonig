...
$name_img_principal 		= $_FILES['img_form_princ']['name'];
	...
	if(!empty($_POST['name_img_principal_actual'])){

		$name_img_a_borrar	= $_POST['name_img_principal_actual'];
...
if ($result_->execute()) {
	...
	$name_img_principal_id = trim(preg_replace('/\s+/', '', $name_img_principal_id));
	echo $name_img_principal_id;

	//Elimino img vieja, ( si hay )
	if(!empty($_POST['name_img_principal_actual'])){
		unlink($carpeta_destino_cpanel_producto_add_img_principal.$name_img_a_borrar);
	}
