...
// para activar o desactivar monto minimo
...
$('#checkbox_tit_active').click(function(){
	...
	if ($('#checkbox_tit_active').is(':checked')) {
		...
		$.when(monto_minimo_activar(id_web,id_cliente)).done(function () {   

		});

	}else{

		$.when(monto_minimo_desactivar(id_web,id_cliente)).done(function () {   
...
function monto_minimo_activar(id_w,id_c){
 
  var deferred = $.Deferred();

  $.ajax({
    "method": "POST",
    "url": "../php/monto_minimo/monto_minimo_activar.php",
    "data": {
...
function monto_minimo_desactivar(id_w,id_c){
 
  var deferred = $.Deferred();

  $.ajax({
    "method": "POST",
    "url": "../php/monto_minimo/monto_minimo_desactivar.php",
    "data": {
              id_web:id_w,id_cliente:id_c
            }
  })   
...
function verificar_campos_vacios(){
  ...
  if( $('#index_input_editlea').val() == ''){
    ...
    toast = 'Monto mínimo actualizado';
...
function enviar_data(id_w,id_c,data_1){
  ...
  $.ajax({
    "method": "POST",
    "url": "../php/monto_minimo/monto_minimo_actualizar.php",
    "data": {
              id_web:id_w,id_cliente:id_c,
              data_1:data_1
            }
